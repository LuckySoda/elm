<template>
  <div id="goods">
    <!-- ref是把dom信息注册到父组件的$refs上 可以通过$refs对象来获取 -->
    ...
    created(){
      ...
      axios.get('/good/goods').then(res =>{
        if(res.data.code === 0){
          ...
          Vue.nextTick(()=>{ //在created加载完后，在dom没有出现之前是不能绑定事件，需要在dom出现之后才能绑定事件，所以此处用Vue.nextTick()
            //dom绑定一个scroll
            this._initscroll()
            //计算foods每一个块的高度
